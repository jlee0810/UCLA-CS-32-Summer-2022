...
void Game::play()
{
    ...
    do
    {
        ...
        msg = "";
        cout << endl;
        cout << "Move (u/d/l/r/su/sd/sl/sr/c/q/p): "; //Updated output message to include p command
        string action;
        getline(cin,action);
        if (action.size() == 0)
            p->stand();
        else
        {
            switch (action[0])
            {
              ...
              case 'r':
                ...
                break;
              case 'p': //Case p is used to print out the Previous board
                m_arena-> thePrevious().showPreviousMoves(); //Prints out the previous board
                cout << "Press Enter to Continue"; //Print out enter to continue
                cin.ignore(10000, '\n');
                m_arena -> display("");
                continue; //continue so that it doesn't count as a move
              case 's':
